...
class ComponentRegistryServiceImpl
    ...
    constructor(
        ...
        private val minorVersionByVersionCache = cacheManager.trackedCache(
            CacheId.MINOR_VERSION_BY_VERSION,
            loaderTracker,
        ) { version: Version ->
            getMinorVersion(version.name, version.project)
        }

        private data class MinorVersionCacheReq(
            val versionName: String,
            val project: Project,
        )

        private val minorVersionByVersionNameAndProjectCache = cacheManager.trackedCache(
            CacheId.MINOR_VERSION_BY_VERSION_NAME_AND_PROJECT,
            loaderTracker,
        ) { req: MinorVersionCacheReq ->
        ...
        private val jiraComponentByProjectAndVersion = cacheManager.trackedCache(
            CacheId.JIRA_COMPONENT_BY_PROJECT_VERSION,
            loaderTracker,
        ) { jiraProjectVersion: JiraProjectVersion ->
            val jiraComponentVersion = clearResponse {
                client
                    .getJiraComponentByProjectAndVersion(jiraProjectVersion.projectKey, jiraProjectVersion.version)
                    .toModel()
            }
            Optional.ofNullable(jiraComponentVersion)
        }

        private val jiraComponentsByProjectCache = cacheManager.trackedCache(
            CacheId.JIRA_COMPONENTS_BY_PROJECT_KEY,
            loaderTracker,
        ) { projectKey: String ->
            clearResponse {
        ...
        private val jiraComponentVersionRangesByProjectCache = cacheManager.trackedCache(
            CacheId.JIRA_COMPONENT_VERSION_RANGES_BY_PROJECT_KEY,
            loaderTracker,
        ) { projectKey: String ->
            clearResponse {
                client
        ...
        private val distributionCacheByJiraProjectVersion = cacheManager.trackedCache(
            CacheId.DISTRIBUTION_BY_JIRA_PROJECT_VERSION,
            loaderTracker,
        ) { jiraProjectVersion: JiraProjectVersion ->
            val distribution = clearResponse {
                client
                    .getDistributionForProject(jiraProjectVersion.projectKey, jiraProjectVersion.version)
                    .toModel()
            }
            Optional.ofNullable(distribution)
        }

        private val distributionByComponentVersionCache = cacheManager.trackedCache(
            CacheId.DISTRIBUTION_BY_COMPONENT_VERSION,
            loaderTracker,
        ) { componentVersion: ComponentVersion ->
            val distribution = clearResponse {
                client
                    .getComponentDistribution(componentVersion.componentName, componentVersion.version)
                    .toModel()
            }
            Optional.ofNullable(distribution)
        }

        private val vcsSettingsByJiraProjectVersionCache = cacheManager.trackedCache(
            CacheId.VCS_SETTINGS_BY_JIRA_PROJECT_VERSION,
            loaderTracker,
        ) { jiraProjectVersion: JiraProjectVersion ->
            val vcsSettings = clearResponse {
                client
                    .getVCSSettingForProject(jiraProjectVersion.projectKey, jiraProjectVersion.version)
                    .toModel()
            }
            Optional.ofNullable(vcsSettings)
        }

        private val vcsSettingsByComponentVersionCache = cacheManager.trackedCache(
            CacheId.VCS_SETTINGS_BY_COMPONENT_VERSION,
            loaderTracker,
        ) { componentVersion: ComponentVersion ->
            val vcsSettings = clearResponse {
                client
                    .getVCSSetting(componentVersion.componentName, componentVersion.version)
                    .toModel()
            }
            Optional.ofNullable(vcsSettings)
        }

        private data class DetailedComponentVersionsCacheRequest(
            val component: String,
            val versions: SortedSet<String>,
        )

        private data class DetailedComponentCacheRequest(
            val component: String,
            val version: String,
        )

        private val detailedComponentVersionsCache = cacheManager.trackedCache(
            CacheId.DETAILED_COMPONENT_VERSIONS,
            loaderTracker,
        ) { req: DetailedComponentVersionsCacheRequest ->
            DetailedComponentVersions(
                req.versions
                    .chunked(50) {
                        client.getDetailedComponentVersions(req.component, VersionRequest(it)).versions.mapValues { entry ->
                            entry.value.toModel()
                        }
        ...
        private val detailedComponentCache = cacheManager.trackedCache(
            CacheId.DETAILED_COMPONENT,
            loaderTracker,
        ) { req: DetailedComponentCacheRequest ->
            client
                .getDetailedComponent(req.component, req.version)
                .toModel()
        }

        private val componentsDistributionByJiraProjectCache = cacheManager.trackedCache(
            CacheId.DISTRIBUTION_BY_PROJECT_KEY,
            loaderTracker,
        ) { projectKey: String ->
            clearResponse {
                client
        ...
        private val componentExistsByJiraProjectVersionCache = cacheManager.trackedCache(
            CacheId.IS_COMPONENT_EXISTS_BY_PROJECT_VERSION,
            loaderTracker,
        ) { jiraProjectVersion: JiraProjectVersion ->
            clearResponse {
        ...
        private val componentExistsByJiraProjectCache = cacheManager.trackedCache(
            CacheId.IS_COMPONENT_EXISTS_BY_PROJECT_KEY,
            loaderTracker,
        ) { projectKey: String ->
            clearResponse {
                client
        ...
        private val jiraComponentByComponentNameAndVersionCache = cacheManager.trackedCache(
            CacheId.JIRA_COMPONENT_BY_COMPONENT_VERSION,
            loaderTracker,
        ) { componentVersion: ComponentVersion ->
            val jiraComponentVersion = clearResponse {
                client
                    .getJiraComponentForComponentAndVersion(componentVersion.componentName, componentVersion.version)
                    .toModel()
            }
            Optional.ofNullable(jiraComponentVersion)
        }

        private val allJiraComponentVersionRangesCache = cacheManager.trackedCache(
            CacheId.ALL_JIRA_COMPONENT_VERSION_RANGES,
            loaderTracker,
        ) { _: Unit ->
            clearResponse {
                client
        ...
        private val detailedComponentVersionCache = cacheManager.trackedCache(
            CacheId.DETAILED_COMPONENT_VERSION,
            loaderTracker,
        ) { componentVersion: ComponentVersion ->
            client
                .getDetailedComponentVersion(componentVersion.componentName, componentVersion.version)
                .toModel()
        }

        private fun createJiraComponentVersionFormatter(): JiraComponentVersionFormatter = JiraComponentVersionFormatter(getVersionNames())

        override fun getAllComponents(): List<Component> = allComponentsCache.get(Unit)!!
        ...
        override fun getComponent(component: String): Optional<Component> = componentsCache.get(component)!!

        override fun getComponentVersionFormatter() = jiraComponentVersionFormatter

        override fun isVersionMinor(version: Version): Boolean = isMinorVersionCache.get(version)!!

        override fun getMinorVersion(version: Version): Optional<String> = minorVersionByVersionCache.get(version)!!

        override fun getMinorVersion(
            versionName: String,
            project: Project,
        ): Optional<String> = minorVersionByVersionNameAndProjectCache.get(MinorVersionCacheReq(versionName, project))!!

        override fun getJiraComponentByProjectAndVersion(jiraProjectVersion: JiraProjectVersion): Optional<JiraComponentVersion> =
            jiraComponentByProjectAndVersion.get(jiraProjectVersion)!!

        override fun getJiraComponentsByProject(projectName: String): Set<String> = jiraComponentsByProjectCache.get(projectName)!!

        override fun getJiraComponentVersionRangesByProject(projectKey: String): Set<JiraComponentVersionRange> =
            jiraComponentVersionRangesByProjectCache.get(projectKey)!!

        override fun getDistribution(jiraProjectVersion: JiraProjectVersion): Optional<Distribution> =
            distributionCacheByJiraProjectVersion.get(jiraProjectVersion)!!

        override fun getDistribution(componentVersion: ComponentVersion): Optional<Distribution> =
            distributionByComponentVersionCache.get(componentVersion)!!

        override fun getVCSSettings(jiraProjectVersion: JiraProjectVersion): Optional<VCSSettings> =
            vcsSettingsByJiraProjectVersionCache.get(jiraProjectVersion)!!

        override fun getVCSSettings(componentVersion: ComponentVersion): Optional<VCSSettings> =
            vcsSettingsByComponentVersionCache.get(componentVersion)!!

        override fun getComponentsDistributionByJiraProject(projectKey: String): Map<String, Distribution> =
            componentsDistributionByJiraProjectCache.get(projectKey)!!

        override fun componentExists(projectVersion: JiraProjectVersion): Boolean =
            componentExistsByJiraProjectVersionCache.get(projectVersion)!!

        override fun componentExists(projectKey: String): Boolean = componentExistsByJiraProjectCache.get(projectKey)!!

        override fun getJiraComponentByComponentNameAndVersion(componentVersion: ComponentVersion): Optional<JiraComponentVersion> =
            jiraComponentByComponentNameAndVersionCache.get(componentVersion)!!

        override fun getAllJiraComponentVersionRanges(): Set<JiraComponentVersionRange> = allJiraComponentVersionRangesCache.get(Unit)!!

        override fun getDetailedComponentVersion(componentVersion: ComponentVersion): DetailedComponentVersion =
            detailedComponentVersionCache.get(componentVersion)!!

        override fun getDetailedComponentVersions(
            component: String,
            versions: Set<String>,
        ): DetailedComponentVersions =
            detailedComponentVersionsCache.get(DetailedComponentVersionsCacheRequest(component, versions.toSortedSet()))!!

        override fun getDetailedComponent(
            component: String,
            version: String,
        ): DetailedComponent = detailedComponentCache.get(DetailedComponentCacheRequest(component, version))!!
        ...
        private fun org.octopusden.octopus.components.registry.core.dto.DetailedComponent.toModel(): DetailedComponent =
            DetailedComponent(
                ...
                buildSystem,
                vcsSettings.toModel(),
                jiraComponentVersion.toModel(),
                detailedComponentVersion.toModel(),
            )
        ...
        private fun JiraComponentVersionDTO.toModel(): JiraComponentVersion {
            ...
            return JiraComponentVersion(
                componentVersion,
                component.toModel(isHotfixEnabled(componentVersion)),
        ...
        private fun JiraComponentDTO.toModel(isHotfixEnabled: Boolean): JiraComponent =
            JiraComponent(
                projectKey,
                displayName,
                this.componentVersionFormat.toModel(),
                this.componentInfo.toModel(),
                technical,
                isHotfixEnabled,
            )

        private fun VersionControlSystemRootDTO.toModel(): VersionControlSystemRoot =
            VersionControlSystemRoot(name, RepositoryType.valueOf(type.name), vcsPath, tag, branch, hotfixBranch)
        ...
        private fun VersionNamesDTO.toModel(): VersionNames = VersionNames(serviceBranch, service, minor)

        private fun VcsRootDateDTO.toModel(): VcsRootWrapper = VcsRootWrapper(root.toModel())

        private fun ComponentInfoDTO.toModel(): ComponentInfo = ComponentInfo(versionPrefix, versionFormat)
        ...
        private fun DistributionDTO.toModel(): Distribution = Distribution(explicit, external, gav ?: "")

        private fun VCSSettingsDTO.toModel(): VCSSettings = VCSSettings(externalRegistry, versionControlSystemRoots.map { it.toModel() })
        ...
        private fun org.octopusden.octopus.components.registry.core.dto.DetailedComponentVersion.toModel(): DetailedComponentVersion =
            DetailedComponentVersion(
                component,
                lineVersion.toModel(),
                minorVersion.toModel(),
                buildVersion.toModel(),
                rcVersion.toModel(),
                releaseVersion.toModel(),
                hotfixVersion?.toModel(),
            )
